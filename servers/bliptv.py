...
def get_video_url( page_url , premium = False , user="" , password="" , video_password="" ):
    logger.info("seriesly.bliptv get_video_url(page_url='%s')" % page_url)

    video_urls = []

    if page_url.startswith("http://blip.tv/play"):    
        redirect = scrapertools.get_header_from_response(page_url,header_to_get="location")
        logger.info("seriesly.bliptv redirect="+redirect)
        
        patron='file\=(.*?)$'
        matches = re.compile(patron).findall(redirect)
        logger.info("seriesly.bliptv matches1=%d" % len(matches))
        
        if len(matches)==0:
            patron='file\=([^\&]+)\&'
            matches = re.compile(patron).findall(redirect)
            logger.info("seriesly.bliptv matches2=%d" % len(matches))
        
        if len(matches)>0:
            url = matches[0]
            logger.info("seriesly.bliptv url="+url)
    ...
    for video_url in video_urls:
        logger.info("seriesly.bliptv %s - %s" % (video_url[0],video_url[1]))
...
def find_videos(text):
    ...
    logger.info("seriesly.bliptv find_videos #"+patronvideos+"#")
