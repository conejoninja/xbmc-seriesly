...
def get_video_url( page_url , premium = False , user="" , password="", video_password="" ):
    logger.info("seriesly.dailymotion get_video_url(page_url='%s')" % page_url)
    video_urls = []
    
    data = scrapertools.cache_page(page_url)
    data = scrapertools.get_match(data,'<param name="flashvars" value="(.*?)"')
    #logger.info("seriesly.dailymotion data="+data)

    #logger.info("data="+data)
    sequence = scrapertools.get_match(data,"sequence=(.*?)$")
    #logger.info("seriesly.dailymotion sequence="+sequence)

    sequence = urllib.unquote(sequence)
    #logger.info("seriesly.dailymotion sequence="+sequence)

    mediaurl = scrapertools.get_match(sequence,'"video_url"\:"([^"]+)"')
    #logger.info("seriesly.dailymotion mediaurl="+mediaurl)
    ...
    for video_url in video_urls:
        logger.info("seriesly.dailymotion %s - %s" % (video_url[0],video_url[1]))
...
def find_videos(data):
    ...
    logger.info("seriesly.dailymotion find_videos #"+patronvideos+"#")
