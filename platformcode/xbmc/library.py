...
# Eliminación de plot y thumnail
    ...
    if addon_name.strip()=="":
        addon_name="plugin://plugin.video.seriesly/"
...
def update(total,errores=0, nuevos=0, serie="No indicada"):
    ...
    if nuevos > 0:
        ...
        if errores == 0:
            actualizar = advertencia.yesno('seriesly' , texto ,'¿Deseas que actualice ahora la Biblioteca?')
            ...
            actualizar = advertencia.yesno('seriesly' , texto , texto2 , '¿Deseas que actualice ahora la Biblioteca?')
        ...
        if errores == 0:
            ...
            texto2 = '(No se pudieron añadir '+str(errores)+' episodios)'
        #advertencia.ok('seriesly',texto,texto2)
        actualizar = False
...
def MonitorSerie ( canal, accion, server, url, serie): 
    ...
    Si se configura para que lo haga seriesly arrancará un proceso al inicio de XBMC
...
def fixStrmLibrary(path = LIBRARY_PATH):
    ...
    plugin cambiaron de:
      plugin://video/seriesly/
    a: 
      plugin://plugin.video.seriesly/
